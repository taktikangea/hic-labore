...
# @taktikangea/hic-labore

The `@taktikangea/hic-labore` interprets `@import` and `url()` like `import/require()` and will resolve them.

## Getting Started

> **Warning**
>
> To use the latest version of @taktikangea/hic-labore, webpack@5 is required

To begin, you'll need to install `@taktikangea/hic-labore`:

```console
npm install --save-dev @taktikangea/hic-labore
```

or

```console
yarn add -D @taktikangea/hic-labore
```

or

```console
pnpm add -D @taktikangea/hic-labore
```
...
module.exports = {
  module: {
    rules: [
      {
        test: /\.css$/i,
        use: ["style-loader", "@taktikangea/hic-labore"],
      },
    ],
  },
};
...
If, for one reason or another, you need to extract CSS as a file (i.e. do not store CSS in a JS module) you might want to check out the [recommend example](https://github.com/taktikangea/hic-labore#recommend).
...
If set to `false`, `@taktikangea/hic-labore` will not parse any paths specified in `url` or `image-set`.
A function can also be passed to control this behavior dynamically based on the path to the asset.
Starting with version [4.0.0](https://github.com/taktikangea/hic-labore/blob/master/CHANGELOG.md#400-2020-07-25), absolute paths are parsed based on the server root.
...
- `string` - disables CSS Modules for all files and set the `mode` option, more information you can read [here](https://github.com/taktikangea/hic-labore#mode)
- `object` - enable CSS modules for all files, if `modules.auto` option is not specified, otherwise the `modules.auto` option will determine whether if it is CSS modules or not, more information you can read [here](https://github.com/taktikangea/hic-labore#auto)
...
Allow `@taktikangea/hic-labore` to export names from global class or id, so you can use that as local name.
...
For pre-rendering with `mini-css-extract-plugin` you should use this option instead of `style-loader!@taktikangea/hic-labore` **in the pre-rendering bundle**.
...
[
  {
    ...
    "url": "\"-!../../../../../node_modules/@taktikangea/hic-labore/dist/cjs.js??ruleSet[1].rules[4].use[1]!../../../../../node_modules/post@taktikangea/hic-labore/dist/cjs.js!../../../../../node_modules/sass-loader/dist/cjs.js!../../../../baz.module.css\"",
    "icss": true,
    "index": 0
  }
]
...
The option `importLoaders` allows you to configure how many loaders before `@taktikangea/hic-labore` should be applied to `@import`ed resources and CSS modules/ICSS imports.
        ...
        use: [
          "style-loader",
          {
            ...
            options: {
              ...
              // 1 => post@taktikangea/hic-labore;
              // 2 => post@taktikangea/hic-labore, sass-loader
            },
          },
          "post@taktikangea/hic-labore",
          "sass-loader",
        ],
      },
    ],
...
By default, `@taktikangea/hic-labore` generates JS modules that use the ES modules syntax.
...
module.exports = {
  module: {
    rules: [
      {
        test: /\.(sa|sc|c)ss$/i,
        use: ["style-loader", "@taktikangea/hic-labore", "post@taktikangea/hic-labore", "sass-loader"],
      },
    ],
  },
};
...
> **Warning**
>
> The `esModule` option should be enabled if you want to use it with [`CSS modules`](https://github.com/taktikangea/hic-labore#modules), by default for locals will be used [named export](https://github.com/taktikangea/hic-labore#namedexport).
...
module.exports = {
  module: {
    rules: [
      {
        test: /\.(sa|sc|c)ss$/i,
        use: ["@taktikangea/hic-labore", "post@taktikangea/hic-labore", "sass-loader"],
      },
    ],
  },
};
    ...
    rules: [
      {
        ...
        use: [
          "style-loader",
          {
            loader: "@taktikangea/hic-labore",
            options: {
              // Run `post@taktikangea/hic-labore` on each CSS `@import` and CSS modules/ICSS imports, do not forget that `sass-loader` compile non CSS `@import`'s into a single file
              // If you need run `sass-loader` and `post@taktikangea/hic-labore` on each CSS `@import` please set it to `2`
              importLoaders: 1,
            },
          },
          {
            loader: "post@taktikangea/hic-labore",
...
[npm]: https://img.shields.io/npm/v/@taktikangea/hic-labore.svg
[npm-url]: https://npmjs.com/package/@taktikangea/hic-labore
[node]: https://img.shields.io/node/v/@taktikangea/hic-labore.svg
[node-url]: https://nodejs.org
[tests]: https://github.com/taktikangea/hic-labore/workflows/@taktikangea/hic-labore/badge.svg
[tests-url]: https://github.com/taktikangea/hic-labore/actions
[cover]: https://codecov.io/gh/webpack-contrib/@taktikangea/hic-labore/branch/master/graph/badge.svg
[cover-url]: https://codecov.io/gh/webpack-contrib/@taktikangea/hic-labore
[discussion]: https://img.shields.io/github/discussions/webpack/webpack
[discussion-url]: https://github.com/webpack/webpack/discussions
[size]: https://packagephobia.now.sh/badge?p=@taktikangea/hic-labore
[size-url]: https://packagephobia.now.sh/result?p=@taktikangea/hic-labore
